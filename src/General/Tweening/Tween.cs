...
using General.Tweening.Equations;
...
namespace General.Tweening
{
	...
	public sealed class Tween : BaseTween<Tween>
	{
		...
		private sealed class _PoolCallback : Pool<Tween>.Callback<Tween>
		{
		...
		private static readonly Pool<Tween>.Callback<Tween> poolCallback = new _PoolCallback();

		private sealed class _Pool : Pool<Tween>
		{
			public _Pool(int initCapacity, Pool<Tween>.Callback<Tween> callback)
				: base(initCapacity, callback)
			{
			}

			protected internal override Tween Create()
			{
				return new Tween();
			}
		}

		private static readonly Pool<Tween> pool = new _Pool(20, poolCallback);
		...
		/// <returns>The current tween, for chaining instructions.</returns>
		public Tween Cast<_T0>(Type targetClass)
		{
			if (IsStarted())
			{
		...
		/// <returns>The current tween, for chaining instructions.</returns>
		public Tween TargetRelative(float targetValue1, float targetValue2, float
			 targetValue3)
		{
			isRelative = true;
			targetValues[0] = IsInitialized() ? targetValue1 + startValues[0] : targetValue1;
